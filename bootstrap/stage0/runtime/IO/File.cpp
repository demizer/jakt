...
ErrorOr<String> File::current_directory()
{
    ...
    auto storage = TRY(StringStorage::create_uninitialized(static_cast<size_t>(expected_size), raw_storage));

    auto final_ret = GetCurrentDirectory(expected_size, raw_storage);

    VERIFY(final_ret == (expected_size - 1));
...
ErrorOr<void> File::make_directory(String path)
{
    ...
    int expected_size = MultiByteToWideChar(CP_UTF8, 0, path.c_string(), static_cast<int>(path.length()), nullptr, 0);
    if (expected_size <= 0)
        return Error::from_errno(GetLastError());
    ...
    int ret = MultiByteToWideChar(CP_UTF8, 0, path.c_string(), static_cast<int>(path.length()), &wide_buffer[wcslen(unc_prefix)], expected_size);
    if (ret <= 0)
        return Error::from_errno(GetLastError());
